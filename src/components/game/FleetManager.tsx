...
import { Plane as PlaneIcon, Clock, Package, ChevronDown, ChevronRight } from 'lucide-react';
import { PlaneType } from '../../types';

export const FleetManager: React.FC = () => {
  const { company, date, tasks, debugUnlockAll, addTask, addNotification, buyPlane } = useGameStore(useShallow(state => {
  ...
  const handleBuyPlane = (plane: PlaneType) => {
    ...
    const task = {
      id: crypto.randomUUID(), // Assuming task needs ID, though addTask might generate it?
      // Wait, let's check addTask in store. Usually it might generate ID. 
      // But looking at previous code, it passed object. 
      // I'll assume addTask handles ID if missing, or I provide it.
      // Ideally I should check `Task` interface.
      type: 'DELIVER_PLANE' as const,
        ...
        {showPending && pendingDeliveries.length > 0 && (
          <div className="space-y-2">
            {pendingDeliveries.map(task => {
              ...
              const daysRemaining = Math.ceil((deliveryDate.getTime() - date.getTime()) / (1000 * 60 * 60 * 24));
              return (
