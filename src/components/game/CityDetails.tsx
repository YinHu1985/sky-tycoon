...
import { Building, Plane, Heart, Building2, Briefcase, Plus, Bus, UtensilsCrossed, Wrench, X, MapPin, ArrowRightLeft } from 'lucide-react';
import { City, Company, PropertyType } from '../../types';
...
const CitySummary: React.FC<CitySummaryProps> = ({ city, company }) => {
  // Calculate relationship
  const relationship = company ? calculateCityRelationship(company, city.id) : 0;
  // Calculate effective attributes
  const { biz, tour } = company ? getCityAttributes(company, city) : { biz: city.biz, tour: city.tour };
...
interface CityDetailsProps {
  cityId: string;
  secondCityId: string | null;
  onCancelSecondCity: () => void;
  onOpenRouteDetails: (routeId: string) => void;
  onOpenRouteCreation?: (cityId: string, secondCityId: string) => void;
}
...
export const CityDetails: React.FC<CityDetailsProps> = ({ 
  ...
  if (secondCity) {
    ...
    // Updated to use 'from' and 'to' instead of 'sourceId' and 'targetId'
    const existingRoute = routes.find(r => 
      (r.from === city.id && r.to === secondCity.id) || 
      (r.from === secondCity.id && r.to === city.id)
    );
          ...
          {existingRoute ? (
            <div className="w-full bg-slate-800 p-2 rounded border border-green-900/50">
              ...
              <div className="flex justify-between text-xs text-slate-300">
                 ...
                 <span className={(existingRoute.stats?.profitLastWeek || 0) >= 0 ? 'text-green-400' : 'text-red-400'}>
  ...
  // Updated to use 'from' and 'to'
  const cityRoutes = routes.filter(r => r.from === cityId || r.to === cityId);
  ...
  const handleBuyProperty = (propertyType: string) => {
  ...
  const handleSellProperty = (propertyId: string, propertyName: string, sellValue: number) => {
  ...
  return (
    <div className="w-[350px] flex flex-col gap-4">
        ...
        {cityProperties.length > 0 ? (
          <div className="space-y-2">
            {cityProperties.map(prop => {
              // Note: OwnedProperty uses 'typeId' in the interface, but legacy code might use 'type'.
              // We updated OwnedProperty interface to include typeId.
              // Assuming runtime objects have typeId properly set or mapped.
              const propType = PROPERTY_TYPES[prop.typeId];
              if (!propType) return null; // Safety check

              const Icon = getPropertyIcon(propType.icon);
              // Use purchaseCost from the updated interface
              const sellValue = (prop.purchaseCost || 0) * 0.5;
      ...
      <div>
        ...
        <div className="space-y-2">
          {Object.values(PROPERTY_TYPES).map((propType: PropertyType) => {
            const Icon = getPropertyIcon(propType.icon);
            const canAfford = company.money >= propType.baseCost;
            const alreadyOwned = cityProperties.some(p => p.typeId === propType.id);
            ...
            return (
              <div key={propType.id} className="bg-slate-700 p-3 rounded border border-slate-600">
                ...
                <div className="text-xs text-slate-400 mb-2 space-y-0.5">
                  ...
                  {propType.maintReduction !== undefined && propType.maintReduction > 0 && (
                    <div className="text-purple-400">-{(propType.maintReduction * 100).toFixed(0)}% flight maintenance</div>
                  )}
                  {propType.serviceReduction !== undefined && propType.serviceReduction > 0 && (
      ...
      <div>
        ...
        <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2">
          {cityRoutes.map(route => {
            // Updated to use 'from' and 'to'
            const isSource = route.from === cityId;
            const otherCityId = isSource ? route.to : route.from;
