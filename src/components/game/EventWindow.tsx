...
export const EventWindow: React.FC = () => {
  ...
  }, [event, activeEvent, dismissEvent]);

  // If no active event or event not found, don't render
  ...
  const handleOptionSelect = (option: Option) => {
    const optionIndex = event.options?.indexOf(option) ?? -1;
    if (optionIndex === -1) return;

    // Apply effects
    applyEventEffects(event, optionIndex, companyId, performAction, date);
                    ...
                    <div className="flex-1">
                      ...
                      {option.effects && option.effects.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-2">
                          {option.effects.map((effect: Effect, idx: number) => (
                            <span
                              key={idx}
                            ...
                            >
                              {effect.type === 'money' && (
                                <>
                                  {effect.amount && effect.amount > 0 ? '💰 +' : '💸 '}
                                  ${Math.abs(effect.amount || 0).toLocaleString()}
                                </>
                              )}
