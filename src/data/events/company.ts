...
They are offering a one-time emergency bailout to keep operations running, but it comes with strings attached.`,
    ...
    modal: true,
    triggers: [
      (state: GameState, company: Company) => company.money < 100000 // Only fires if money is very low (< $100k)
    ],
    mtth_modifiers: [
      { 
        factor: 0.1, // 10x more likely (effectively immediate)
        condition: (state: GameState, company: Company) => company.money < -1000000 // If deeply in debt
      }
    ],
    options: [
      {
...
Their salary demands are high, but they're offering a 5-year contract that could transform your airline.`,
    ...
    modal: true,
    triggers: [
      (state: GameState, company: Company) => company.money > 2000000 // Must have some cash
    ],
    mtth_modifiers: [
      {
        factor: 0.5, // 2x more likely
        condition: (state: GameState, company: Company) => (company.fame || 0) > 80 // If famous
      },
      {
        factor: 2.0, // 2x less likely
        condition: (state: GameState, company: Company) => (company.fame || 0) < 20 // If unknown
      }
    ],
    options: [
      {
  ...
  },
  {
    ...
    modal: true,
    triggers: [
       (state: GameState, company: Company) => !!company && (company.fame || 0) > 30 // Must be somewhat known
    ],
    mtth_modifiers: [
       { factor: 0.5, condition: (state: GameState, company: Company) => !!company && (company.serviceEffort || 0) > 80 } // High service attracts celebs
    ],
    options: [
       {
