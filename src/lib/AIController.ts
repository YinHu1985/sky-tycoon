...
import { calculateDistance, calculateFlightTime } from './utils';
import { calculateFrequency, calculateOptimalRouteConfig, OptimalRouteConfig } from './economy';
import { GameState, Company, PlaneType, City } from '../types';
...
export function processAI(gameState: AIContext) {
  try {
    ...
    companies.forEach(company => {
      try {
        ...
        if ((company.maintenanceEffort || 0) < 50) {
            performAction(company.id, 'UPDATE_EFFORTS', { 
                maintenanceEffort: 60, 
                serviceEffort: company.serviceEffort || 50, 
                prBudget: company.prBudget || 0
            });
        }
      } catch (err) {
...
function buyNewPlane(company: Company, performAction: Function, date: Date) {
...
function findBestRouteFromSource(company: Company, sourceId: string, sortedPlaneTypes: PlaneType[]) {
  ...
  for (const plane of sortedPlaneTypes) {
      ...
      company.routes.forEach(r => {
          if (r.from === sourceId) existingTargets.add(r.to);
          if (r.to === sourceId) existingTargets.add(r.from);
      });

      // Calculate distance to all other cities and filter by range
      const potentialTargets: RouteCandidate[] = CITIES.filter(c => 
          c.id !== sourceId && 
      ...
      for (const candidate of topCandidates) {
          const config: OptimalRouteConfig = calculateOptimalRouteConfig(
              company,
              sourceId,
              candidate.city.id,
              plane.id,
          ...
          // console.log(`[AI-${company.code}]   -> Target: ${candidate.city.name} (${Math.round(candidate.dist)}km). Profit: ${Math.round(config.profit || 0)}`);

          if ((config.profit || 0) > bestProfit) {
              bestProfit = config.profit || 0;
...
function openNewRoute(company: Company, performAction: Function, date: Date) {
  // 1. Identify valid source cities (HQ or existing route endpoints)
  const connectedCities = new Set(company.routes.map(r => r.from).concat(company.routes.map(r => r.to)));
  ...
  const sortedPlaneTypes = freePlaneTypes
    .map(id => PLANE_TYPES.find(p => p.id === id))
    .filter((p): p is PlaneType => p !== undefined)
