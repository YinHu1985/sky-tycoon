...
import { Company, City, OwnedProperty, PropertyType, Modifier, ModifierContext } from '../types';
 ...
 */
export const getModifiersForTarget = (company: Company, target: string, context: ModifierContext = {}): Modifier[] => {
  ...
  if (company.activeModifiers) {
     const activeModifiers = company.activeModifiers;
    const eventModifiers = activeModifiers.filter(mod => {
 ...
 */
export const applyModifiers = (baseValue: number, modifiers: Modifier[]): number => {
  let multiplier = 1.0;
  let flatBonus = 0;
 ...
 */
export const expireModifiers = (company: Company, currentDate: Date): Modifier[] => {
 ...
 */
const getBuiltInModifiers = (company: Company, target: string, context: ModifierContext): Modifier[] => {
  const modifiers: Modifier[] = [];
  ...
  if (target === 'loadFactor') {
    ...
    const fameMultiplier = 0.8 + (fame / 100) * 0.4;
    modifiers.push({
      source: 'fame',
      type: 'multiplier',
      target: 'loadFactor',
      value: fameMultiplier,
      description: `Fame bonus (${fame.toFixed(0)})`
    });
  }
  ...
  if (target === 'maintenanceCost' || target === 'idleCost') {
    ...
    const maintEffort = company.maintenanceEffort || 0;
    const maintMultiplier = 0.5 + (maintEffort / 100) * 1.3;
    modifiers.push({
      source: 'maintenanceEffort',
      type: 'multiplier',
      target,
      value: maintMultiplier,
      description: `Maintenance effort (${maintEffort}%)`
    });
  }
  ...
  if (target === 'flightCost') {
    ...
    const serviceEffort = company.serviceEffort || 0;
    const serviceMultiplier = 0.7 + (serviceEffort / 100) * 0.8;
    modifiers.push({
      source: 'serviceEffort',
      type: 'multiplier',
      target: 'flightCost',
      value: serviceMultiplier,
      description: `Service quality (${serviceEffort}%)`
    });
  }
 ...
 */
export const calculateFame = (currentFame: number, maintenanceEffort: number, serviceEffort: number, prBudget: number, company: Company): number => {
 ...
 */
export const calculateCityRelationship = (company: Company, cityId: string): number => {
  ...
  const connectedRoutes = company.routes.filter(
    r => r.from === cityId || r.to === cityId // Note: Route interface uses 'from' and 'to', but modifiers.js used 'sourceId' and 'targetId'. Check Route interface.
  );
  // Wait, Route interface in types/index.ts uses 'from' and 'to'. But in JS code it seemed to use sourceId/targetId?
  // Let's assume types/index.ts is correct and JS code was maybe using sourceId/targetId.
  // Actually, let's double check Route interface.
  ...
  if (company.properties) {
    ...
    cityProperties.forEach(prop => {
      const propType = PROPERTY_TYPES[prop.typeId]; // PropertyType uses typeId in OwnedProperty? Check types.
      if (propType) {
 ...
 */
export const getCityAttributes = (company: Company, city: City): { biz: number, tour: number } => {
 ...
 */
const getPropertyLoadFactorBonus = (company: Company, sourceId?: string, targetId?: string): number => {
  ...
  if (sourceId) {
    ...
    sourceProps.forEach(prop => {
      const propType = PROPERTY_TYPES[prop.typeId];
 ...
 */
const getMaintenanceCenterReduction = (company: Company, sourceId?: string, targetId?: string): number => {
  if (!company.properties) return 0;
 ...
 */
const getServiceCostReduction = (company: Company, sourceId?: string, targetId?: string): number => {
  if (!company.properties) return 0;
 ...
 */
export const calculatePropertyFinancials = (company: Company) => {
